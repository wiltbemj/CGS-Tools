# CGS-Tools
Description of methods to access CGS models results using kaipy and tiegcmpy

# Introduction
This repository provides basic instructions for setting up and using the kaipy and tiegcmpy packages for accessing results from the MAGE models.  At the moment these software package are in beta phase as we would appreciate feedback on the tools as well as contributions.  Questions and feedback can be directed to Michael Wiltberger via email to wiltbemj@ucar.edu.
...
# Installing kaipy and tiegcmpy

We have made kaipy and tiegcmpy installable via pip from the PyPi index.  We are currently working a more tight integration with conda, but for now pip is the prefered method doing installations.  

First step is to create virtual python environment to use with the installation of the tools.  You may chose any name for the environment we are going to use `cgs-mage-tools` for this tutorial. Using the conda package manmager

```
()$ conda create -n cgs-mage-tools python=3.8
()$ conda activate cgs-mage-tools
(cgs-mage-tools)$ pip install kaipy
```

The tiegcmpy package utilizes [Cartopy](https://scitools.org.uk/cartopy/docs/latest/) for adding contentianal information to the plots.  Since installing the dependices for Cartopy can be tricky we've found it helpful to install via the conda-forge channels before installing tiegcmpy into your environment.

```
(cgs-mage-tools)$ conda install -c conda-forge cartopy
(cgs-mage-tools)$ pip install tiegcmpy
```

After completing this step you will have a conda virtual environment that has the kaipy and tiegcm packages as well as some scirpts for producing quicklook plots and conducting analysis of MAGE model results.  

If you want to use this environment with JupyterLab you need to use ipykernels to make it available to JupyterLab.  Starting by installing `nb_conda_kernels` into your base conda virtual environment.
...
```
(base)$ conda activate cgs-mage-tools
(cgs-mage-tools)$ conda install ipykernel
(cgs-mage-tools)$ conda deactivate
```
...
```
(cgs-mage-tools)$ pip show -f kaipy | grep Location | awk '{print $2"/kaipy"}'
```
...
```
(cgs-mage-tools)$ setenv CGSKAIPY `pip show -f kaipy | grep Location | awk '{print $2"/kaipy"}'`
```
...
After starting your JupyterLab server you will need to open the KaijuExamplePlot notebook file in the `$CGSKAIPY/tutorial/` directory.
